...
func setup(grid: BattleGrid, mover: UnitMover, camera: BattleCamera, tile_visual_manager: TileVisualManager) -> void:
	_grid = grid
	_unit_mover = mover
	_camera = camera
...
func apply_effect(target, effect_id: EffectId.Id, ticks: int = -1) -> void:
	if _is_immune(target, effect_id):
		# TODO: Is effect removed form immune target?
...
func apply_tile_entry_effects(actor: BattleActor, tile: BattleTileData, suppressed: Array[EffectId.Id] = []) -> void:
	if tile == null or tile.active_effects.is_empty():
		return
	var context = EffectContext.create(_grid, _unit_mover, self)
	for instance in tile.active_effects.duplicate():
		if suppressed.has(instance.effect_id):
			continue
		var handler = EffectRegistry.get_handler(instance.effect_id)
		if handler != null:
			await handler.on_actor_entered_tile(actor, tile, instance, context)

func apply_tile_event_effects(actor: BattleActor, event: TileEvent, tile: BattleTileData) -> void:
	if tile == null or tile.active_effects.is_empty():
		return
	var context = EffectContext.create(_grid, _unit_mover, self)
	for instance in tile.active_effects.duplicate():
...
func apply_damage(actor: BattleActor, amount: int, play_reaction: bool = true) -> void:
	await actor.apply_damage(amount, play_reaction)
