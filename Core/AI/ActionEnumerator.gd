...
static func enumerate_actions(unit: Unit, origin: BattleTileData, pathfinder: Pathfinder, context: TurnContext) -> Array[ActionCandidate]:
	...
	for ability in abilities:
		...
		DebugLog.ai("considering %s — %d reachable targets, %d valid" % [ability.ability_name, context.reachable_target_cells.size(), context.reachable_target_cells.values().count(true)])
